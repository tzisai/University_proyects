    ...
    while(seguir){
        ...
        switch(elect){
            case '1':{
                ...
                cout<<"Ingresa una cadena binaria (ej. 010101), o 'x' para salir:\n> ";
                string cadena; cin>>cadena;
                if(cadena=="x"||cadena=="X") break;

                State1 estado=Q0;
                cout<<"q0"; // estado inicial
                for(char c:cadena){
                    if(c!='0'&&c!='1'){ cout<<"\nEntrada invalida en cadena.\n"; break; }
                    estado=transicion1(estado,c);
                    cout<<" -> "<<name1(estado);
                }
                bool acept=(estado==Q0);
                cout<<"\nCadena "<<cadena<<" "<<(acept?"ACEPTADA":"RECHAZADA")
                    <<" (estado final "<<name1(estado)<<")\n\n";
                break;
            }
            case '2':{
                cout<<"\n--- Automata ejercicio 3 ---\n";
                cout<<"Ingresa una cadena binaria (ej. 10110), o 'x' para salir:\n> ";
                string cadena; cin>>cadena;
                if(cadena=="x"||cadena=="X") break;
                ...
                for(char c:cadena){
                    if(c!='0'&&c!='1'){ cout<<"\nEntrada invalida en cadena.\n"; break; }
                    estado=transicion2(estado,c);
                    cout<<" -> "<<name2(estado);
                }
                bool acept=(estado==P1);
                cout<<"\nCadena "<<cadena<<" "<<(acept?"ACEPTADA":"RECHAZADA")
                    <<" (estado final "<<name2(estado)<<")\n\n";
                break;
            }
