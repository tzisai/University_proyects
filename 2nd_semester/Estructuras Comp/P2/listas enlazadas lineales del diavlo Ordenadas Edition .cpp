...
Objetivo:Crear un progrma que tenga dentro de sus menus las operaciones básicas de ls listas enlazadas lineales PERO 
ahora ordenadas.
...
void ordenacion(nodo*&lista){
	
	if (lista == nullptr || lista->siguiente == nullptr) {
        // La lista está vacía o tiene solo un elemento, por lo que está ordenada
        return;
    }
	nodo* nodo_ant = lista;
    nodo* nodo_act = lista;
   ...
   while(nodo_act!=nullptr){
    	if((nodo_act->datote)>nodo_ant->datote){
    		
    		aux = nodo_ant->datote;
            nodo_ant->datote = nodo_act->datote;
            nodo_act->datote = aux;
