        ...
        for condicion in regla["if"]:
            ...
            if variable in inputs and inputs[variable] != "":
                if str(inputs.get(variable)) != str(valor):  # Convertir a cadena para comparar
                    cond = False
                    break
        if cond:  # Este indent es clave para validar la regla completa
...
def ask_forward_inputs(kb):
    ...
    elif kb == kb_vehiculos:
        preguntas = [
            ("vehicleType", "¿Qué tipo de vehículo es? (cycle/automobile)"),
            ("num_wheels", "¿Cuántas ruedas tiene?"),
            ("motor", "¿Tiene motor? (si/no)"),
            ("size", "¿Cuál es el tamaño del vehículo? (small/medium/large)"),
            ("num_doors", "¿Cuántas puertas tiene?")
        ]
    ...
    def siguiente_pregunta(index=0):
        if index < len(preguntas):
            ...
            resultado = forward_chaining(kb, inputs_user)
            textbox2.config(state="normal")
            textbox2.delete(1.0, tk.END)
            textbox2.insert(tk.END, f"El resultado es: {resultado}")
