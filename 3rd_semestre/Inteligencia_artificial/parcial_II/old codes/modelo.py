...
# Funcion para eliminar el archivo csv
def borrar_archivo():
    try:
...
def contar_coincidencias(imagen, kernels):
    ...
    for kernel in kernels:
        ...
        # Contar coincidencias (puedes definir esto como quieras, aquí se usa una suma de valores)
        count = np.sum(resultado > 0)  # Aumenta el contador si el valor es mayor que 0
        coincidencias.append(count)
    return coincidencias

# Define algunos kernels (puedes ajustar esto según tus necesidades)
kernels = [
    np.array([[1, 0, -1], [1, 0, -1], [1, 0, -1]]),  # Ejemplo de kernel
    np.array([[1, 1, 1], [0, 0, 0], [-1, -1, -1]]),   # Otro kernel
    np.array([[0, 1, 0], [1, -4, 1], [0, 1, 0]]),     # Kernel Laplaciano
...
coincidencias_list = []  # Lista para almacenar el número de coincidencias por kernel
...
invalid_labels = []
invalid_validity = []
invalid_coincidencias = []  # Lista para almacenar coincidencias de firmas no válidas
